...
@csrf_exempt
def view_student_TA(request):
    if request.method != 'POST':
        allTA = LeaveReportStudent.objects.all()
        context = {
            'allTA': allTA,
            'page_title': 'TA Applications From Students'
        }
        return render(request, "hod_template/student_TA_view.html", context)
    else:
        id = request.POST.get('id')
        status = request.POST.get('status')
        if (status == '1'):
            ...
            status = -1
        try:
            TA = get_object_or_404(LeaveReportStudent, id=id)
            TA.status = status
            TA.save()
            return HttpResponse(True)
        except Exception as e:
